<?php 
   session_start();
   $connect = mysqli_connect("127.0.0.1", "root", "Starkova24", "vhelp"); 
   
   
   $orgs_id = $_SESSION['id'];
   
   
   $query = mysqli_query($connect, "SELECT * from gigs WHERE orgs_id='{$orgs_id}' ");
   
     $gig_info = $query-> fetch_assoc();
   
   $org = mysqli_query($connect, "SELECT * from orgs WHERE id='{$orgs_id}' ");
     $org_info = $org-> fetch_assoc();
   
   
   
   
      ?>
<!DOCTYPE html>
<html lang="en" >
   <head>
      <meta charset="UTF-8">
      <title></title>
      <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/uikit/3.0.0-rc.23/css/uikit.min.css'>
      <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
      <link rel="stylesheet" href="./style.css">
      <link rel="stylesheet" type="text/css" href="../fonts/css/all.css">
      <style type="text/css">
         .user_link{
         border:none;
         background: none;
         color: #03b1fc;
         font-size: 16px;
         margin: 0;
         padding: 0;
         }
         .user_link:hover{
         text-decoration: underline;
         }
         .decision{
         width: 100px;
         height: 30px;
         margin: 5px;
         border:none;
         background: none;
         color: white;
         transition: 0.2s ease;
         }
         .decision:hover{
         transform: scale(1.02);
         }
         .accept{
         background-color: #32a852
         }
         .decline{
         background-color: #d10d0d;
         }
         .thumb{
         padding: 0;
         }
         .end{
         position: absolute;
         margin-top: -40px;
         margin-left: 240px;
         }
         .thumb span{
         font-size: 20px
         }
      </style>
   </head>
   <body>
      <div uk-sticky class="uk-navbar-container tm-navbar-container uk-active uk-light ">
         <div class="uk-container uk-container-expand uk-background-secondary">
            <nav uk-navbar>
               <div class="uk-navbar-left">
                  <a id="sidebar_toggle" class="uk-navbar-toggle" uk-navbar-toggle-icon></a>
                  <a href="#" class="uk-navbar-item uk-logo">
                  VHELP
                  </a>
               </div>
               <div class="uk-navbar-right uk-light">
                  <ul class="uk-navbar-nav">
                     <li>
                        <a href="../index.php"> На главную </span></a>
                     </li>
                     <li>
                        <a href="../gig_submit/index.php"> Добавить gig </span></a>
                     </li>
                     <li class="uk-text-bold">
                        <a href="../session_kill.php" class="uk-text-danger"> выйти </span></a>
                     </li>
                  </ul>
               </div>
            </nav>
         </div>
      </div>
      <div id="sidebar" class="tm-sidebar-left uk-background-secondary uk-light">
         <center>
            <div class="user">
               <img id="avatar" width="100" class="uk-border-circle" src="https://image.flaticon.com/icons/svg/167/167750.svg" />
               <div class="uk-margin-top"></div>
               <h3 class="uk-text-truncate"><?php echo $org_info['name'] ?></h3>
               <span id="status" class="uk-margin-bottom uk-label uk-label-success"><?php echo $org_info['position'] ?></span>
               <div id="email" class="uk-text-truncate"><?php echo $org_info['email'] ?></div>
            </div>
            <br />
         </center>
      </div>
      <div class="content-padder content-background">
         <div class="uk-section-small uk-section-default header">
            <div class="uk-container uk-container-large">
               <h1></span><?php echo $org_info['org_name'] ?></h1>
               <p>
                  <?php echo $org_info['org_type'] ?>
               </p>
            </div>
         </div>
         <div class="uk-section-small">
            <div class="uk-container opportunities">
               <ul class="uk-subnav uk-subnav-pill" uk-switcher>
                  <li><a href="#">Сейчас в прогрессе</a></li>
                  <li><a href="#">Новые заявки</a></li>
                  <li><a href="#">Законченные работы</a></li>
               </ul>
               <ul class="uk-switcher uk-margin">
                  <li>
                     <div class="jobs ">
                        <?php 
                           $gigs = mysqli_query($connect, "
                            SELECT * from in_action 
                           
                            INNER JOIN gigs ON in_action.gigs_id = gigs.id
                           
                            WHERE in_action.orgs_id = '{$orgs_id}' and in_action.status = 'active'
                           
                             ")or die(mysqli_error($connect));
                           
                           while ($row = $gigs->fetch_assoc()) {
                           
                           ?>
                        <div class="card_ " id="<?php echo $row['skill_id']?>">
                           <div class="thumb <?php echo $row['skill_id'] ?>">
                              <i class="fas <?php echo $row['skill_icon'] ?>"></i> 
                              <span><?php echo $row['skill_name'] ?></span>
                              <form method="post" action="end_gig.php">
                                 <input style="display: none" name="gigs_id" value="<?php echo $row['id']?>">
                                 <?php 
                                    $user = mysqli_query($connect, "SELECT * from users WHERE id='".$row['users_id']."' ");
                                    $user_info = $user-> fetch_assoc();?>
                                 <input style="display: none" name="users_id" value="<?php echo $user_info['id']?>">
                                 <button class="decision decline end" type="submit">Закончить</button>
                              </form>
                           </div>
                           <div class="infos">
                              <h2 class="title"><?php echo $row['title'] ?></h2>
                              <h3 class="company">
                              Работает: 
                              <form method="post" action="../user_profile/index.php">
                                 <input style="display: none" name="users_id" value="<?php echo $user_info['id']; ?>">
                                 <input style="display: none" name="skill_id" value="<?php echo $row['skill_id']; ?>">
                                 <input class="user_link" type="submit" value="<?php echo $user_info['name']; ?>">
                              </form>
                              <div class="description-block">
                                 <div class="time">
                                    <span class="hours"><?php echo $row['hours'] ?></span>
                                    <span>часов в неделю</span>
                                 </div>
                                 <div class="skills">Удаленная работа: <?php echo $row['remote_work'] ?></div>
                                 <div class="places">мест осталось: <?php echo $row['seats'] ?></div>
                              </div>
                           </div>
                        </div>
                        <?php } ?>
                     </div>
                  </li>
                  <li>
                     <table class="uk-table uk-table-middle uk-table-divider">
                        <thead>
                           <tr>
                              <th class="uk-table-shrink">ID работы</th>
                              <th>Кандидат</th>
                              <th>Номер телефона</th>
                              <th>Специальность</th>
                              <th>Анкета</th>
                              <th>Решение</th>
                           </tr>
                        </thead>
                        <tbody>
                           <?php 
                              $apps = mysqli_query($connect, "
                               SELECT * from users INNER JOIN apps ON users.id = apps.users_id WHERE apps.orgs_id = '{$orgs_id}' ") or die(mysqli_error($connect));
                              
                              while ($fetch_apps = $apps->fetch_assoc()) {
                              
                              ?>          
                           <tr>
                              <td><?php echo $fetch_apps['gigs_id'] ?></td>
                              <td><?php echo $fetch_apps['name'] ?></td>
                              <td><?php echo $fetch_apps['phone'] ?></td>
                              <td><?php echo $fetch_apps['major'] ?></td>
                              <td>
                                 <form method="post" action="../user_profile/index.php">
                                    <input style="display: none" name="users_id" value="<?php echo $fetch_apps['users_id']; ?>">
                                    <input style="display: none" name="skill_id" value="skill5">
                                    <input class="user_link" type="submit" value="link">
                                 </form>
                              </td>
                              <td>
                                 <form method="post" action="insert.php">
                                    <input style="display: none" name="users_id" value="<?php echo $fetch_apps['users_id'] ?>">
                                    <input style="display: none" name="gigs_id" value="<?php echo $fetch_apps['gigs_id'] ?>">
                                    <input style="display: none" name="status" value="active">
                                    <input class="decision accept" type="submit" value="Принять"></input>
                                 </form>
                                 <form method="post" action="delete.php">
                                    <input style="display: none" name="users_id" value="<?php echo $fetch_apps['users_id'] ?>">
                                    <input style="display: none" name="gigs_id" value="<?php echo $fetch_apps['gigs_id'] ?>">
                                    <input class="decision decline" type="submit" value="Отклонить"></input>
                                    <!-- <button class="uk-button uk-button-default" type="button">Отклонить</button> -->
                                 </form>
                              </td>
                           </tr>
                           <?php } ?>
                        </tbody>
                     </table>
                  </li>
                  <li>
                     <div class="jobs ">
                        <?php 
                           $gigs = mysqli_query($connect, "
                            SELECT * from in_action 
                           
                            INNER JOIN gigs ON in_action.gigs_id = gigs.id
                           
                            WHERE in_action.orgs_id = '{$orgs_id}' and in_action.status = 'inactive'
                           
                             ")or die(mysqli_error($connect));
                           
                           while ($row = $gigs->fetch_assoc()) {
                           
                           ?>
                        <div class="card_ " id="<?php echo $row['skill_id']?>">
                           <div class="thumb <?php echo $row['skill_id'] ?>">
                              <i class="fas <?php echo $row['skill_icon'] ?>"></i> 
                              <span><?php echo $row['skill_name'] ?></span>
                           </div>
                           <div class="infos">
                              <h2 class="title"><?php echo $row['title'] ?></h2>
                              <h3 class="company">
                              <?php 
                                 $user = mysqli_query($connect, "SELECT * from users WHERE id='".$row['users_id']."' ");
                                 $user_info = $user-> fetch_assoc();?>
                              Работал: 
                              <form method="post" action="../user_profile/index.php">
                                 <input style="display: none" name="users_id" value="<?php echo $user_info['id']; ?>">
                                 <input style="display: none" name="skill_id" value="<?php echo $row['skill_id']; ?>">
                                 <input class="user_link" type="submit" value="<?php echo $user_info['name']; ?>">
                              </form>
                              <div class="description-block">
                                 <div class="time">
                                    <span class="hours"><?php echo $row['hours'] ?></span>
                                    <span>часов в неделю</span>
                                 </div>
                                 <div class="skills">Удаленная работа: <?php echo $row['remote_work'] ?></div>
                                 <div class="places">мест осталось: <?php echo $row['seats'] ?></div>
                              </div>
                           </div>
                        </div>
                        <?php } ?>
                     </div>
                  </li>
               </ul>
            </div>
         </div>
      </div>
      </div>
      <!-- partial -->
      <script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js'></script>
      <script src='https://cdnjs.cloudflare.com/ajax/libs/uikit/3.0.0-rc.23/js/uikit.min.js'></script>
      <script src='https://cdnjs.cloudflare.com/ajax/libs/uikit/3.0.0-rc.23/js/uikit-icons.min.js'></script>
      <script src='https://cdnjs.cloudflare.com/ajax/libs/jquery.transit/0.9.12/jquery.transit.min.js'></script>
      <script src='https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.3/Chart.bundle.js'></script><script  src="./script.js"></script>
   </body>
</html>